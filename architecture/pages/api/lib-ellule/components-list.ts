...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const componentsListPath = path.resolve('pages/api/lib-ellule/system/Library', 'componentsList.json');
    fs.writeFileSync(componentsListPath, JSON.stringify(componentsList, null, 2));

    const dataFilePath = path.resolve('pages/api/lib-ellule/system/Library', 'data.json');
    const response = await fetch('https://plasmic-api.agence-scroll.com/data.json');

...
// Type pour le style d'un header de colonne
type HeaderStyleObject = {
  backgroundColor?: string;
  textColor?: string;
  borderRadius?: string;
...
interface KanbanProps {
  ...
  headerStyle?:
    | HeaderStyleObject
    | Record<string, HeaderStyleObject>
    | ((columnId: string) => HeaderStyleObject);
  taskColors?: {
    backgroundColor?: string;
    textColor?: string;
    borderColor?: string;
...
const transformApiTask = (apiTask: ApiTask): Task => {
  return {
    ...
    associated_operation: apiTask.associated_operation
  };
};
...
// Type guard pour savoir si headerStyle est un objet d'indexation par colonne
function isHeaderStyleRecord(
  style: KanbanProps['headerStyle']
): style is Record<string, HeaderStyleObject> {
  return (
    typeof style === 'object' &&
    style !== null &&
    !Array.isArray(style) &&
    Object.keys(style).some((k) => typeof (style as any)[k] === 'object')
  );
}

const defaultHeaderStyle: HeaderStyleObject = {
      ...
      <DragDropContext onDragEnd={onDragEnd}>
        <div className={`${styles.kanbanContainer}`} 
          ...
          {data.columnOrder.map((columnId) => {
            ...
            const backgroundColor = fixedColumn?.backgroundColor ||
              (typeof headerStyle === 'function'
                ? headerStyle(columnId)?.backgroundColor
                : (headerStyle as any)?.stylesParColonne && (headerStyle as any).stylesParColonne[columnId]
                  ? (headerStyle as any).stylesParColonne[columnId].backgroundColor
                  : isHeaderStyleRecord(headerStyle) && headerStyle[columnId]
                    ? headerStyle[columnId].backgroundColor
                    : (headerStyle as any)?.backgroundColor);

            // Gestion du champ stylesParColonne pour Plasmic (backgroundColor et textColor uniquement)
            const stylesParColonne = (headerStyle as any).stylesParColonne;
            let columnHeaderStyle: HeaderStyleObject =
              typeof headerStyle === 'function'
                ? headerStyle(columnId)
                : stylesParColonne && stylesParColonne[columnId]
                  ? {
                      ...defaultHeaderStyle,
                      ...(headerStyle as HeaderStyleObject),
                      ...(['backgroundColor', 'textColor'].reduce((acc, key) => {
                        if (stylesParColonne[columnId][key]) acc[key] = stylesParColonne[columnId][key];
                        return acc;
                      }, {} as any))
                    }
                  : isHeaderStyleRecord(headerStyle) && headerStyle[columnId]
                    ? headerStyle[columnId]
                    : (headerStyle as HeaderStyleObject);
            if (!columnHeaderStyle || typeof columnHeaderStyle !== 'object') {
              columnHeaderStyle = defaultHeaderStyle;
            }
              ...
              >
                <div 
                  ...
                  style={{
                    ...
                    color: columnHeaderStyle.textColor,
                    borderRadius: columnHeaderStyle.borderRadius,
                    textAlign: columnHeaderStyle.textAlign,
                    fontSize: columnHeaderStyle.fontSize,
                    fontWeight: columnHeaderStyle.fontWeight,
                    padding: columnHeaderStyle.padding,
                    fontFamily: columnHeaderStyle.fontFamily,
                    textTransform: columnHeaderStyle.uppercase ? 'uppercase' : 'none',
                    position: 'sticky',
                    top: 0,
                    zIndex: 1
                  }}
                                ...
                                >
                                  ...
                                  <div className="flex items-center justify-between mb-3">
                                    <div style={{ 
                                      ...
                                      No <div style={{ fontSize: '12px'}}>{task.id}</div>
                                    </div>
                                    {task.priority !== "SUIVI" && (
                                  ...
                                  {task.associated_operation && (
                                    <div style={{ marginBottom: '4px' }}>
                                      <div style={{ fontSize: '8px', fontWeight: '400', color: '#131013', fontFamily: 'Manrope', marginBottom: '2px' }}>Opération :</div>
                                      <div style={{ fontSize: '12px', fontWeight: '400', color: '#131013', fontFamily: 'Manrope' }}>{task.associated_operation}</div>
                                    </div>
                                  )}

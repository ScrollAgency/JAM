...
const handleMajComponents = async () => {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
    if (selectedItem === "Maj Components") {
      setMessage(`Lancer l'écriture du fichier components.ts en fonction du contenu du dossier plasmic-library`);
      setError(null);
    }
  }, [selectedItem]);
  ...
  return (
    <>
      ...
      {selectedItem === "Maj Components" && (
        <div className={styles.versionInfo}>

          {!isComponentsGenerating && (
            <button onClick={handleMajComponents} className={styles.updateButton} type="button">
              Générer
            </button>
          )}

          {isComponentsGenerating && <p>🔄 Téléchargement en cours...</p>}
          {error && <p className={styles.errorMessage}>{error}</p>}
        </div>
      )}
    </>

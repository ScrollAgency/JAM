...
import styles from './api/lib-ellule/first-install.module.css';
...
const FirstInstallPage = ({ folderExists }: FirstInstallPageProps) => {
  ...
  const startInstallation = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch('/api/lib-ellule/first-install', { method: 'POST' });
      if (!response.ok) throw new Error("Erreur lors du lancement de l'installation");

      const postInstallResponse = await fetch('/api/lib-ellule/post-install', { method: 'POST' });
  ...
  React.useEffect(() => {
    const interval = setInterval(() => {
      fetch('/api/lib-ellule/install-log')
        .then(res => res.json())
        .then(setLogs)
        .catch(console.error);
    }, 1000);
...
export async function getServerSideProps() {
  const folderPath = path.resolve(process.cwd(), 'plasmic-library');
